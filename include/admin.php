	...
	class archivesPostTabWidget_Admin extends archivesPostTabLib {    
		...
		*/
		...
		//	$instance = wp_parse_args( $instance, $this->_config );   
			...
			require( $this->getArchivesPostTabTemplate( 'admin/admin_widget_settings.php' ) );
...
if ( ! class_exists( 'archivesPostTabLicenseConfig_Admin' ) ) {
	...
	class archivesPostTabLicenseConfig_Admin extends archivesPostTabLib {    
		...
		public function archivesposttab_manage_license() {
			...
			if( isset( $_POST['btnAct'] ) && trim($_POST['btnAct']) != "" &&  isset( $_GET["page"] ) && trim( $_GET["page"] ) == "archivesposttab_settings" ){
				...
				if( $act_key == "" ) {
					...
					wp_redirect(site_url()."/wp-admin/edit.php?post_type=avptab_archives&page=archivesposttab_settings&st=11");
					die();
				...
				wp_redirect( site_url()."/wp-admin/edit.php?post_type=avptab_archives&page=archivesposttab_settings&st=".$license_data->st );
				die();				 
		...
		public function avptab_display_plugin_settings_admin_page() {
			...
			if( isset( $_REQUEST["st"] ) && trim( $_REQUEST["st"] ) != "" ) { 
				...
				if( $_st == '1' ) {
					...
					$_message = __( 'Your site url is not registered from ikhodal.com or invalid license key, Please add your site url and get license key from ikhodal.com account if you have already purchased Archive Post Tabs plugin for wordpress.', 'archivesposttab');	
						
				} else if( $_st == '4' ) {
					$_message = __( 'License key has been deactivated.', 'archivesposttab');
						
				} else if( $_st == '5' ) {
					$_message = __( "Invalid license key. Please get valid licence key from ikhodal.com account. If you have already purchased 'Archive Post Tabs' plugin for wordpress.", 'archivesposttab');
			...
			?> 
				<div  class="wrap">
					...
					<p><?php _e( "Activate/Deactivate 'Archive Posts Tabs' plugin using provided license key.", 'archivesposttab' ); ?></p>
					...
					<form method="post" action="<?php echo site_url(); ?>/wp-admin/edit.php?post_type=avptab_archives&page=archivesposttab_settings"> 
		...
		public function avptab_add_plugin_admin_menu() {
			...
			add_submenu_page('edit.php?post_type=avptab_archives', __( 'License Key', 'archivesposttab'), __( 'License Key', 'archivesposttab'), 'manage_options', 'archivesposttab_settings',	array( $this, 'avptab_display_plugin_settings_admin_page' )); 			
...
add_action( 'widgets_init', 'initrichcategoryarchivestabs' ); 
function initrichcategoryarchivestabs() {
