<?php  
/**
 * Register shortcode and render post data as per shortcode configuration. 
 */ 
if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly   
if ( ! class_exists( 'archivesPostTabWidget' ) ) { 
	class archivesPostTabWidget extends archivesPostTabLib {
	 
	   /**
		* Constructor method.
		*
		* Run the following methods when this class is loaded
		*
		* @access  public
		* @since   1.0
		*
		* @return  void
		*/ 
		public function __construct() {
		
			add_action( 'init', array( &$this, 'init' ) ); 
			parent::__construct();
			
		}  
		
	   /**
		* Load required methods on wordpress init action 
		*
		* @access  public
		* @since   1.0
		*
		* @return  void
		*/ 
		public function init() {
		
			add_action( 'wp_ajax_avptab_getTotalPosts',array( &$this, 'avptab_getTotalPosts' ) );
			add_action( 'wp_ajax_avptab_getPosts',array( &$this, 'avptab_getPosts' ) ); 
			add_action( 'wp_ajax_avptab_getMorePosts',array( &$this, 'avptab_getMorePosts' ) );
			
			add_action( 'wp_ajax_nopriv_avptab_getTotalPosts', array( &$this, 'avptab_getTotalPosts' ) );
			add_action( 'wp_ajax_nopriv_avptab_getPosts', array( &$this, 'avptab_getPosts' ) ); 
			add_action( 'wp_ajax_nopriv_avptab_getMorePosts', array( &$this, 'avptab_getMorePosts' ) ); 
			
			add_shortcode( 'archivesposttab', array( &$this, 'archivesPostTab' ) ); 
			
		} 
		
	  	

		 
	   /**
		* Render the shortcode
		*
		* @access  public
		* @since   1.0
		*
		* @param   array   $params  Shortcode configuration options from the admin settings
		* @return  string  Render tab html
		*/
		public function archivesPostTab( $params = array() ) { 	
		
			if(isset($params["id"]) && trim($params["id"]) != "" && intval($params["id"]) > 0) {
				$archivesposttab_id = $params["id"]; 
				$avptab_shortcode = get_post_meta( $archivesposttab_id ); 
				
				foreach ( $avptab_shortcode as $sc_key => $sc_val ) {			
					$avptab_shortcode[$sc_key] = $sc_val[0];			
				} 
				
				if(!isset($avptab_shortcode["date_format"]))	
					$avptab_shortcode["date_format"] = 0; 
				if(!isset($avptab_shortcode["number_of_post_display"]))	
					$avptab_shortcode["number_of_post_display"] = 0;
				if(!isset($avptab_shortcode["category_id"]))	
					$avptab_shortcode["category_id"] = 0;
					
				$this->_config = shortcode_atts( $this->_config, $avptab_shortcode ); 
				$this->_config["vcode"] =  "uid_".md5(md5(json_encode($this->_config)).$this->getUCode());	
				
			} else {
			
				$this->init_settings();
				
				// default option settings
				foreach($this->_config as $default_options => $default_option_value ){
				  if(!isset($params[$default_options]))
					$params[$default_options] = $default_option_value["default"];
				}
				
				if(count($params)>0) {
					$this->_config = shortcode_atts( $this->_config, $params ); 
				}
				if(!isset($this->_config["date_format"]))	
					$this->_config["date_format"] = 0; 
				if(!isset($this->_config["category_id"]))	
					$this->_config["category_id"] = 0;
					
				$this->_config["vcode"] =  "uid_".md5(md5(json_encode($this->_config)).$this->getUCode());		
				
			}
			
			/**
			* Load template according to admin settings
			*/
			ob_start();
			
			require( $this->getArchivesPostTabTemplate( "fronted/front_template.php" ) );
			
			return ob_get_clean(); 
		
		}   
		
	   /**
		* Loads the next posts via ajax request
		*
		* @access  public
		* @since   1.0
		* 
		* @return  void Displays searched posts HTML to load more pagination
		*/	
		public function avptab_getMorePosts() {
		
			global $wpdb, $wp_query; 
			
		   /**
			* Check security token from ajax request
			*/
			check_ajax_referer($this->_config["security_key"]["security_key"], 'security' );
			
			$_date_format = ( isset( $_REQUEST["date_format"] )?esc_attr( $_REQUEST["date_format"] ):0 ); 
			$_total = ( isset( $_REQUEST["total"] )?esc_attr( $_REQUEST["total"] ):0 );
			$category_id = ( isset( $_REQUEST["category_id"] )?esc_attr( $_REQUEST["category_id"] ):0 );
			$post_search_text = ( isset( $_REQUEST["post_search_text"] )?esc_attr( $_REQUEST["post_search_text"] ):"" );  
			$_limit_start = ( isset( $_REQUEST["limit_start"])?esc_attr( $_REQUEST["limit_start"] ):0 );
			$_limit_end = ( isset( $_REQUEST["number_of_post_display"])?esc_attr( $_REQUEST["number_of_post_display"] ):avptab_number_of_post_display ); 
			$avptab_default_category = ( isset( $_REQUEST["avptab_default_category"] )?esc_attr( $_REQUEST["avptab_default_category"] ):$category_id );	
		   /**
			* Fetch posts as per search filter
			*/	
			$_result_items = $this->getSqlResult( $_date_format, $avptab_default_category, $post_search_text, $_limit_start, $_limit_end );
		  
			require( $this->getArchivesPostTabTemplate( 'fronted/ajax_load_more_posts.php' ) );	
			
			wp_die();
		}    
		
	   /**
		* Load more posts via ajax request
		*
		* @access  public
		* @since   1.0
		* 
		* @return  object Displays searched posts HTML
		*/
		public function avptab_getPosts() {
		
		   global $wpdb; 
			
		   /**
			* Check security token from ajax request
			*/	
		   check_ajax_referer( $this->_config["security_key"]["security_key"], 'security' );	   
		   
		   require( $this->getArchivesPostTabTemplate( 'fronted/ajax_load_posts.php' ) );	
		   
  		   wp_die();
		
		}
		 
	   /**
		* Get post list with specified limit and filtered by date, category and search text
		*
		* @access  public
		* @since   1.0 
		*
		* @param   int     $category_id 	  Selected category ID 
		* @param   string  $post_search_text  Post name or any search keyword to filter posts
		* @param   float   $date_format		  Set the date format
		* @param   int     $_limit_end		  Limit to fetch post ending to given position
		* @return  object  Set of searched post data
		*/
		public function getPostList( $category_id, $post_search_text, $date_format, $_limit_end ) {
			
		   /**
			* Check security token from ajax request
			*/	
			check_ajax_referer( $this->_config["security_key"]["security_key"], 'security' );		
			
		   /**
			* Fetch data from database
			*/
			return $this->getSqlResult( $date_format, $category_id, $post_search_text, 0, $_limit_end );
			 
		} 
		
	}
	
}
new archivesPostTabWidget();